...
export const usePlayground = defineStore("playground", () => {
    const lang = useLocalStorage<BundledLanguage>(
        "decondenser-playground-lang",
        "rust",
    );
    const theme = useLocalStorage<BundledTheme>(
        "decondenser-playground-theme",
        "dark-plus",
    );
    ...
    const input = useLocalStorage("decondenser-playground-input", "");
    ...
    void (async () => {
        ...
        const webGrammars = allGrammars.grammars.filter(
            (grammar) => grammar.categories?.includes("web"),
            0,
        );
        ...
        function fetchSample(id: string): Promise<string | undefined> {
            if (!samplesCache.has(id)) {
                samplesCache.set(
                    id,
                    fetch(
                        `https://raw.githubusercontent.com/antfu/textmate-grammars-themes/main/samples/${id}.sample`,
                    )
                        .then((r) => r.text())
                        .catch((e) => {
                            console.error(e);
        ...
        if (typeof window !== "undefined") {
            ...
            function run(): void {
                // output.value = `<pre>${input.value}</pre>`;
