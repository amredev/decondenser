...
import { $init, decondenser } from "decondenser";

await $init;

const play = usePlayground();
const currentLang = computed(() =>
    play.allLanguages.find((i) => i.name === play.lang),
);
const currentTheme = computed(() =>
    play.allThemes.find((i) => i.name === play.theme),
);
...
const decon = new decondenser.Decondenser({});

function format(text: string) {
    console.time("formatting");
    const formatted = decon.format(text);
...
<template>
    <div style="display: flex">
        <div
            class="language-ts vp-adaptive-theme transition-none! playground"
            shadow
            :style="[
                play.preStyle,
                { colorScheme: currentTheme?.type || 'inherit' },
                { flex: 1 },
            ]"
        >
            <div
            ...
            >
                ...
                >
                    ...
                    >
                        <option
                            v-for="lang in play.allLanguages"
                            :key="lang.name"
                            :value="lang.name"
                        >
                            {{ lang.name }}
                ...
                >
                    ...
                    >
                        <option
                            v-for="theme in play.allThemes.filter(
                                (i) => i.type === 'light',
                            )"
                            :key="theme.name"
                            :value="theme.name"
        ...
        <div style="flex: 1; max-width: 50%">
            <pre><code>{{ format(textAreaRef?.value ?? '') }}</code></pre>
        </div>
    </div>
</template>
